...
// 接続（初回/再接続）時に同じroomへ再join
socket.on('connect', () => {
  if (currentRoomId) socket.emit('join', { roomId: currentRoomId });
  if (!viewOnly) document.querySelectorAll('button').forEach(b => (b.disabled = false));
...
socket.on('disconnect', () => {
  if (!viewOnly) document.querySelectorAll('button').forEach(b => (b.disabled = true));
...
function sendControl(action, extra = {}) {
  ...
  if (!socket.connected) return; // 未接続は送らない
